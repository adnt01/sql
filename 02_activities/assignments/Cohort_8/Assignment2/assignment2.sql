...
SELECT
*,
TRIM( SUBSTR(product_name, NULLIF(INSTR(product_name, '-')+1,1))) AS product_description
FROM product;
...
DROP TABLE IF EXISTS temp.rank_qty;
CREATE TEMP TABLE temp.rank_qty AS
	SELECT 
	vendor_id,
	market_date,
	product_id,
...
-- Approach1 keep only quantity_rank = 1 :
DELETE FROM rank_qty WHERE quantity_rank != 1;
select * from rank_qty;

UPDATE product_units
SET current_quantity = COALESCE(
        (SELECT quantity 
         FROM rank_qty 
         WHERE rank_qty.product_id = product_units.product_id),
        0
    );
select * from product_units;

-- Approach 2 add quantity_rank == 1 to where clause 
UPDATE product_units
SET current_quantity = COALESCE(
        (SELECT quantity 
         FROM rank_qty 
         WHERE rank_qty.product_id = product_units.product_id and quantity_rank == 1),
        0
    );
select * from product_units;
